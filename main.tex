%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Seminar Presentation about the Paper
%%%% Toussaint, Marc, and Amos Storkey. "Probabilistic inference for solving discrete and continuous state Markov Decision Processes." Proceedings of the 23rd international conference on Machine learning. 2006.
%%
%% Presented for Computer Science M.Sc. Seminar @ Ben Gurion University
...
\begin{frame}<1-3>{Markov Decision Processes}
	...
	\note{WRITE MDP Input Functions on the Board!}
	\note<1>[item]{So, let's remember the definition of a Markov Decision Process:}
	\note<1>[item]{State transition probability is the same as usual, where the states and actions can be either continuous or discrete.}
	...
	\note<2>[item]{The action probability $P(a_t \mid x_t ; \policy)$ is parameterized by a policy \policy{} parameter such that $P(a_t \mid x_t = i; \policy) = \pi_{ai}$}
% 	\note<2>[item]{The action probability is dependent on a parameter \policy which we call the Policy.}
	...
	\note<3>[item]{This is an unrolled Dynamic Bayesian Network. Each node is a random variable, and each arrow denotes a causal relationship.}
    \note<3>[item]{The graph encodes conditional independence relations between sets of variables. Examples ($a_1 \bot a_0 \mid x_1$). These conditional independencies are what allow us to factor the probability distribution into compact forms.}
	\note<3>[item]{Our approach is to cast the problem of solving an MDP into a problem of optimizing the parameters of a graphical model, which includes all the future states and actions as latent variables.}
...
\frame{{Mixture of MDPs}
	...
	\note[item]{To achieve exact equivalence between solving the MDP and standard likelihood maximization we will define a likelihood function that is proportional to the expected future return, and then maximize it.}
	...
	\note[item]{The only observables are the starting-state and reward vars.}
% 	\note[item]{In probabilistic computing, the Likelihood function is one of the fundamental elements which we can use to solve problems.}
	\note{DRAW Figure 2. on the board! For general T. ($\forall T$).}
}
\makeatother
...
\begin{frame}{Defining the likelihood}
    ...
    \end{corollary}
    \note<1->[item]{What we are trying to do is solve an MDP, which is to find a policy that maximizes the future reward. The approach here is to define a likelihood function which is proportional to $V^\policy(i)$, such that by maximizing the likelihood we will also maximize the expected future reward.}
...
\begin{frame}{An EM-algorithm for computing the optimal policy}
    ...
    \pause
    \begin{block}{E-step}
        For a given \policy{}, compute posteriors $P(x_{1:T}, a_{1:T} \mid x_0 = A, r = 1, T; \policy)$ and $P(T \mid x_0 = A, r = 1; \policy)$.
    \end{block}
    \pause
    \begin{block}{M-step}
...
\begin{frame}<1-5>{E-step: forward-backward in all MDPs synchronously}
    \only<1>{
    ...
    \note<1>[item]{So now we don't write the names of the random variables.}
    \note<1>[item]{This reads: the probability of transitioning from state i to state j, given action a.}
    \note<1>[item]{This reads: the probability of transitioning from state i to state j, given policy \policy{}.}
    \note<2, 3>[item]{We will only show the algorithm for the discrete case, but later we will discuss another inference technique based on the EM algorithm which works for the continuous case.}
    ...
    \note<5>[item]{Since the $\alpha, \beta$'s are independent of $T$, we can perform $\alpha$- and $\beta$- propagation in parallel, incrementing $t$ and $\tau$ synchronously, and can retrieve the $\alpha$'s and $\beta$'s for all MDPs of any finite time $T$.}
    \note<5>[item]{Although we introduce a mixture of MDPs we only have to perform a single forward and backward sweep.}
    \note<5>[item]{This procedure is, from the point of view of ordinary HMMs, quite unusual - it is possible because in our specific setup we only condition on the very first ($x_0 = A$) and very last state ($r = 1$).}
...
\begin{frame}{M-step: the policy update}
	...
	$Q(\pi^*, \pi) = \sum_T \sum_{x_{0:T}, a_{0:T}} P(x_{0:T}, a_{0:T}, T \mid r = 1; \policy) \log P(r=1, x_{0:T}, a_{0:T}, T; \policy^*)$
	\end{definition}
	}
	\only<2-3>{
	\begin{fct}
	...
	\begin{smashedalign}
	    P(r=1 \mid x_0=i; \policy) = &\sum\limits_{aj} P(r=1 \mid a_t=a, x_t = j ; \policy^*) \pi_{aj}^*\\&\cdot P(x_t = j \mid x_0 = i ; \policy^*)
	\end{smashedalign}
	\end{hwv}
	}
	\only<4>{
...
% 	\end{hwv}
	\begin{block}{Thus}
	   Maximizing the \emph{action-conditioned likelihood}
	     \begin{columns}[b]
              \begin{column}{2.5cm}
                      $\pi_{ai}^{*} = \delta_{a=a^{*}(i)}$
              \end{column}
	...
	\note<1->[item]{The standard M-step in an EM-algorithm maximizes the \emph{expected complete log-likelihood} w.r.t. the new parameters $\pi^*$, where expectations over the latent variables $(T, x_{0:T}, a_{0:T}$) are taken w.r.t. the posterior given the old parameters $\pi$.}
...
\begin{frame}{Relation to Policy Iteration}
    ...
    \begin{alertblock}{Thus,}
         the EM-algorithm using exact inference and belief representation is  effectively \emph{equivalent} to Policy Iteration but computes the necessary quantities in a different way.
    \end{alertblock}
    }
    \only<9>{
    \begin{alertblock}{However,}
         when using approximate inference or belief representations, the EM-algorithm and Policy Iteration are qualitatively \emph{different}.
    \end{alertblock}
    }
    ...
    \note[item]{Additionally, yields time,state,action posteriors which have no traditional analogue.}
...
\frame{{Discrete maze}
	...
	\note[item]{Figure 3(a) displays the posterior state visiting probabilities generated by our probabilistic inference planner (PIP) for a problem where rewards are given when some goal state is reached. }
}
\makeatother
...
\frame{{Discrete maze}
	...
	\note[item]{Figure 3(b) displays the probability of reaching the goal $P(r=1 \mid x_) = A; \policy$ against these costs for the same two start/goal state configurations.}
	\note[item]{Computational costs are measure by the number of evaluations of the environment $p(j \mid i, a)$ needed during the planning procedure. }
	...
	\note[item]{+++: In contrast to VI and PI, the PIP algorithm takes considerable advantage of knowing the start state in this planning scenario: the forward propagation allows for the pruning and the early decision on cutoff times of the E-step as described in Appendix A. It should thus not be a surprise and not overstated that PIP is significantly more efficient in this specific scenario. Certainly, some kind of forward propagations could also be introduced for VI or PI to achieve similar efficiency. Nonetheless, our approach provides a principled way of pruning by exploiting the computation of proper posteriors.}
...
\begin{frame}{Stochastic optimal control - 'walker'}
    ...
    \begin{alertblock}{}
    \begin{itemize}
        \item $P(x' \mid u, x) = \mathcal{N}(x', \phi(u, x), Q + 0\cdot (|u|/\mu)^2 I)$ is \emph{transitions}.
    ...
    \note[item]{Figure 4ab shows the learned control policy \policy{} and the forward simulation ($\alpha$) given this policy by displaying the covariance ellipses for $\alpha_{0:T}(x)$ after $k=3$ EM iterations.}
...
\begin{frame}{Stochastic optimal control - 'golfer'}
    \begin{figure}
        \centering
        \includegraphics[width=0.9\textwidth]{figs/fig_4cd_control_example.png}
        % \caption{}
        \label{fig:my_label}
    \end{figure}
    \vspace*{-0.6cm}
    \begin{alertblock}{}
    \begin{itemize}
        \item $P(x' \mid u, x) = \mathcal{N}(x', \phi(u, x), Q + (|u|/1)^2 I)$ is \emph{transitions}.
        \item $\alpha_0(x) = \mathcal{N}(x, (0,0), .01I)$ \quad \quad \quad \quad \quad \quad \quad is   \emph{start-state}.
        \item $P(r=1 \mid x) = \mathcal{N}(x, (1,1), diag(.0001,.1))$ is \emph{goal}.
        \item $\phi(u, x) = x + .1u$ \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad is \emph{control-law}.
    \end{itemize}
    \end{alertblock}
    \note[item]{Next we can investigate what the effect of control-dependent noise is without a constraint on the amplitude of $u$.}
    \note[item]{Figure 4cd displays results (after $k=3$ iters) for $\mu=1$}
    \note[item]{As before, (left) learned policy, (right) forward simulations $\alpha$'s)}
    \note[item]{The process actually resembles a golf player: the stronger the hit, the more noise.}
    \note[item]{The optimal strategy is to hit fairly hard at the beginning, hopefully coming closer to the goal, such that later a number of smaller and more precise hits can be made.}
    \note[item]{The reason for the small control signals around the goal region is that small steps have much more accuracy and reward epxectation is already fairly large for just the $x$-coordinate being close to 1.}
\end{frame}
...
\begin{frame}{Stochastic optimal control - 'phase space'}
    \begin{figure}
        \centering
        \includegraphics[width=0.9\textwidth]{figs/fig_4ef_control_example.png}
        % \caption{}
        \label{fig:my_label}
    \end{figure}
    \vspace*{-0.6cm}
    \begin{alertblock}{}
    \begin{itemize}
        \item $P(x' \mid u, x) = \mathcal{N}(x', \phi(u, x), Q + (|u|/10)^2 I)$ is \emph{transitions}.
        \item $\alpha_0(x) = \mathcal{N}(x, (0,0), .001I)$ \quad \quad \quad \quad \quad \quad \quad is   \emph{start-state}.
        \item $P(r=1 \mid x) = \mathcal{N}(x, (1,1), .001I)$  \quad \quad \quad \quad is \emph{goal}.
        \item $\phi(x, u) = (x_1 + .1x_2, x_2 + .1u)$ \quad \quad \quad \quad \quad \quad is \emph{control-law}.
    \end{itemize}
    \end{alertblock}

    \note[item]{Finally, we think of $x$ being a phase space and consider the dynamics $\phi(x, u) = (x_1 + .1x_2, x_2 + .1u)$ where $u$ is the 1-dimensional acceleration of the velocity $x_2$, and $x_1$ is a position.}
    \note[item]{This time we set the start and goal to (0,0) and (1,0) respectively, both with variance .001 and choose $\mu=10$.}
    \note[item]{The agent learns to approach a new position under phase space dynamics}
    \note[item]{Figure 4ef display the result and show nicely how the learned control policy approaches the new position on the $x$-axis by first gaining and then reducing velocity.}
\end{frame}
